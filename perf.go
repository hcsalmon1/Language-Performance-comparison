...
var bestRoute [NUMBER_OF_CITIES]int

var distanceMatrix = [NUMBER_OF_CITIES][NUMBER_OF_CITIES]int{
...
func RunFullRecursive() {

	time.Sleep(1 * time.Second)

	start := time.Now()

	var currentRoute [NUMBER_OF_CITIES]int
	var visited [NUMBER_OF_CITIES]bool
...
func FindShortestPath(currentRoute [NUMBER_OF_CITIES]int, visited [NUMBER_OF_CITIES]bool, level int, lastIndex int, currentDistance int) {
	...
	for i := 0; i < NUMBER_OF_CITIES; i++ {
		...
		if !visited[i] {
			...
			newDistance := currentDistance + distanceMatrix[lastIndex][i]
