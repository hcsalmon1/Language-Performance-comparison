...
let bestRoute = Array(NUMBER_OF_CITIES).fill(0);

function findShortestPath(currentRoute, visited, level, lastIndex, currentDistance) {
    ...
    for (let i = 0; i < NUMBER_OF_CITIES; i++) {
        if (!visited[i]) {
            ...
            const newDistance = currentDistance + distanceMatrix[lastIndex][i];
...
function runFullRecursive() {
    const start = Date.now();

    const currentRoute = Array(NUMBER_OF_CITIES).fill(0);
    const visited = Array(NUMBER_OF_CITIES).fill(false);
